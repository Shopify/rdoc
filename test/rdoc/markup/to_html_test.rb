...
<pre>&gt;&gt;&gt;  word</pre>
  ...
  def test_gen_url_markdown_anchor
    # Markdown-style anchor links work directly with GitHub-style header IDs
    assert_equal '<a href="#hello">link</a>',
                 @to.gen_url('#hello', 'link')
  end

  def test_gen_url_markdown_anchor_with_hyphens
    # Markdown-style anchor with hyphens passes through as-is
    assert_equal '<a href="#hello-world">link</a>',
                 @to.gen_url('#hello-world', 'link')
  end

  def test_gen_url_markdown_anchor_preserves_label_prefix
    # Legacy links with label- prefix are preserved for backward compatibility
    assert_equal '<a href="#label-hello">link</a>',
                 @to.gen_url('#label-hello', 'link')
  end

  def test_convert_tidy_link_markdown_anchor
    # Test full conversion flow with tidy link notation
